...
class Graph2Property():
    ...
    def create_network(self):
        ...
        if( self.FLAGS.model == 'GCN' ):
            self.Z, self._X= blocks2.encoder_gcn_deep(self.X, self.A, self.batch_size, latent_dim, self.skip_connection)
        elif( self.FLAGS.model == 'GAT' ):
            self.Z, self._X, self._A  = blocks2.encoder_gat_deep(self.X, self.A, self.batch_size, latent_dim, self.skip_connection)
        self._P = blocks2.predictor_mlp(self.Z)
